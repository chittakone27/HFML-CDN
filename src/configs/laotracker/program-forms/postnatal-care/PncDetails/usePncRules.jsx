...
const usePncRules = () => {
  ...
  useEffect(() => {
    ...
    const foundEvent = currentEvents.find((ce) => {
      ...
      const foundServiceType = ce.dataValues.find(
        (dv) => dv.dataElement === SERVICES_TYPE && dv.value
      );
      return foundPs && foundServiceType;
    });
    if (foundEvent) {
      const foundServiceType = foundEvent.dataValues.find(
        (dv) => dv.dataElement === SERVICES_TYPE
      );
      changeDataValue(
        currentEvent.event,
        SERVICES_TYPE,
        foundServiceType.value
      );
    }
    ///////////////////////////////////////////////////////////////////////////////////
    ...
    if (dataValues[PNC_VISIT_NUMBER] === "1") {
      ...
      currentHiddenFields = currentHiddenFields.filter(
        (f) => f !== PNC2_WITHIN_2_3DAYS
      );
      currentHiddenFields = currentHiddenFields.filter(
        (f) => f !== PNC3_WITHIN_7_14DAYS
      );
    }
    ...
    if (
      dataValues[PNC_VISIT_NUMBER] &&
      parseInt(dataValues[PNC_VISIT_NUMBER]) > 6
    ) {
    ...
    const diastolicBloodPressure = dataValues[DIASTOLIC_BLOOD_PRESSURE]
      ? parseInt(dataValues[DIASTOLIC_BLOOD_PRESSURE])
      : null;
    const systolicBloodPressure = dataValues[SYSTOLIC_BLOOD_PRESSURE]
      ? parseInt(dataValues[SYSTOLIC_BLOOD_PRESSURE])
      : null;
    ...
    if (dataValues[EXCLUSIVE_BREASTFEEDING] === "false") {
      currentHiddenFields = currentHiddenFields.filter(
        (f) => f !== REASON_NOT_BREASTFEEDING
      );
      currentHiddenFields = currentHiddenFields.filter(
        (f) => f !== WHAT_TO_FEED_BABY
      );
    } else {
    ...
    if (
      dataValues[RECEIVED_IFA_TABLETS] === "true" ||
      dataValues[RECEIVED_IFA_TABLETS] === "false"
    ) {
      currentDisabledFields.push(RECEIVED_IFA_TABLETS);
      currentHiddenFields = currentHiddenFields.filter(
        (f) => f !== HOW_MANY_IFA_TABLETS
      );
    } else {
      currentHiddenFields.push(HOW_MANY_IFA_TABLETS);
      currentDisabledFields = currentDisabledFields.filter(
        (cdf) => cdf !== RECEIVED_IFA_TABLETS
      );
    ...
    if (dataValues[RECEIVED_VITAMIN_B1] === "true") {
      currentHiddenFields = currentHiddenFields.filter(
        (f) => f !== HOW_MANY_VITAMIN_B1_TABLETS
      );
    } else {
  ...
  useEffect(() => {
    if (previousPncEvents.length > 0) {
      const foundReceivedIFA = findDataValue(
        previousPncEvents[0].dataValues,
        RECEIVED_IFA_TABLETS
      );
      changeDataValue(
        currentEvent.event,
