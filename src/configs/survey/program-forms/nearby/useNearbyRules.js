...
const CAN_TAVEL_BY_CAR = "NI37vfjXk6J";    // Can travel by car
...
const NEED_HUMAN_CARRY_BIKE_ID = "xcFzGlfAZNW"; // Need human assistance (Yes/No)
const FEES_CARRY_BIKE_ID = "ttoaHSY8sTB";       // Fees for human assistance
...
const useNearbyRules = () => {
  ...
  useEffect(() => {
    ...
    if (hasBoat1) {
      ...
      const showTriggerVal = dv(SHOW_TRIGGER_ID);
      const showNorm = norm(showTriggerVal);
      if (showNorm !== "true" && showNorm !== "yes") {
        hiddenFields[SHOW_IF_TRUE_ID] = true;
      }
    }
