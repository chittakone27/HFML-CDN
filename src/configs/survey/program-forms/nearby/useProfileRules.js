...
const useProfileRules = () => {
  ...
  const A = useMemo(() => toObj(currentTei?.attributes || []), [currentTei?.attributes]);
  ...
  useEffect(() => {
    const assignations = {};
    const nextId = composeFacilityId(A[TEA.districtId], A[TEA.hfType], A[TEA.seqNum]);
    if (typeof nextId !== "undefined") assignations[TEA.facilityId] = nextId;

    setProps((p) => ({ ...p, assignations }));
  }, [A[TEA.districtId], A[TEA.hfType], A[TEA.seqNum]]);
