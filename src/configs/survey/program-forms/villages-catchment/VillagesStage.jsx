...
const VHV_SECTION_EN = "Village Health Volunteer (VHV)";
const VHV_SECTION_LO = "ອາສາສະໝັກສາທາລະນະສຸກບ້ານ (ອສບ)";
...
const VillagesStage = () => {
  ...
  const dynamicDryTitle = useMemo(() => {
    ...
    return isLao
      ? `ການເດີນທາງໃນລະດູແລ້ງ ຈາກ ຫ້ອງການບ້ານ ${from} ຫາ ${to}`
  ...
  const dynamicRainyTitle = useMemo(() => {
    ...
    const to =
      ...
      (isLao ? "ສະຖານທີ່ບໍລິການສຸຂະພາບ" : "the health facility");
    return isLao
      ? `ການເດີນທາງໃນລະດູຝົນ ຈາກ ຫ້ອງການບ້ານ ${from} ຫາ ${to}`
