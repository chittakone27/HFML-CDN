...
const HealthFacilitySelectorNoState = ({
  ...
  useEffect(() => {
    ...
    if (init?.ph) {
      setL2({ type: "PH", id: init.ph });
    } else if (init?.ch) {
      setL2({ type: "CH", id: init.ch });
    } else if (init?.district) {
      setL2({ type: "DHO", id: init.district });
    } else {
      setL2({ type: "", id: "" });
    }
  ...
  const onL2Change = (e) => {
    ...
    setL2({ type: picked.type, id: picked.id });
    setL3(nextL3);
  ...
  const onL3Change = (e) => {
    const id = e.target.value || "";
    const picked =
      l3Options.find((o) => o.id === id) || { type: "", id: "" };
  ...
  return (
    <Box sx={{ display: "grid", rowGap: ROW_GAP }}>
      ...
      >
        ...
        >
          {l2Options.map((o) => (
            <MenuItem key={`${o.type}-${o.id}`} value={o.id}>
              {o.label}
            </MenuItem>
          ))}
        </Select>
