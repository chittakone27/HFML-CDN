...
const FilePicker = ({ onChange, disabled, type, value }) => {
    ...
    async function getFileFromValue() {
      try {
        const response = await pull(
          `/api/fileResources/${value.split("-")[2]}`
        );
                ...
                <img
                  src={
                    file
                      ...
                      : `${baseUrl}/api/events/files?dataElementUid=${
                          value.split("-")[0]
                        }&eventUid=${value.split("-")[1]}`
                  }
                  alt="uploaded"
                  style={{
                    width: "100%",
