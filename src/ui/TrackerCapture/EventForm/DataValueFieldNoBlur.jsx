...
const DataValueFieldNoBlur = (props) => {
  ...
  const { actions, layout } = useTrackerCaptureStore(
    (state) => ({ actions: state.actions, layout: state.layout }),
    shallow
  );
  const { changeDataValue, changeEventProperty, setLayout } = actions;
  const foundPde = currentProgramStage.programStageDataElements.find(
  ...
  if (
    currentEvent.orgUnit !== orgUnit.id &&
    currentEvent.status !== "SCHEDULE"
  ) {
    currentDisabled = true;
  }
  ...
  return (
    <div className="input-field-container">
      ...
      <Input
        ...
        maxDate={
          foundDe.allowFutureDate ? undefined : moment().format("YYYY-MM-DD")
        }
        initialDate={
          foundDe.valueType === "AGE" ? currentEvent.eventDate : undefined
        }
        accept={
          foundDe.valueType === "DATE" ||
          foundDe.valueType === "AGE" ||
          foundDe.valueType === "ORGANISATION_UNIT"
            ? (value) => {
                changeDataValue(currentEvent.event, dataElement, value);
              }
            : undefined
